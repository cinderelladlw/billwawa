#include "thread-pool.h"
  ...
  //内部接口
static void *_tp_work_thread(void *pthread);
static void *_tp_manage_thread(void *pthread);

  //tp_thread_pool的指针函数实现。
...
TPBOOL tp_init(tp_thread_pool *this){
	...
	for(i=0;i<this->min_th_num;i++){
		...
		err = pthread_create(&this->thread_info[i].thread_id, NULL, _tp_work_thread, this);
		if(0 != err){
	...
	//creat manage thread
	err = pthread_create(&this->manage_thread_id, NULL, _tp_manage_thread, this);
	if(0 != err){
...
void tp_process_job(tp_thread_pool *this, tp_work *worker, tp_work_desc *job)
{
	...
	for(i=0;i<this->cur_th_num;i++){
		...
		if(!this->thread_info[i].is_busy){
			printf("tp_process_job: %u thread idle, thread id is %u\n", i, this->thread_info[i].thread_id);
			...
			printf("tp_process_job(): informing idle working thread %u, thread id is %u\n", i, this->thread_info[i].thread_id);
	...
	printf("tp_process_job(new): informing idle working thread %u, thread id is %u\n", i, this->thread_info[i].thread_id);
	pthread_cond_signal(&this->thread_info[i].thread_cond);
	return;	
}
...
static TPBOOL tp_add_thread(tp_thread_pool *this) {
	...
	err = pthread_create(&new_thread->thread_id, NULL, _tp_work_thread, this);
	if(0 != err){
		free(new_thread);
		return FALSE;
	}
...
static TPBOOL tp_delete_thread(tp_thread_pool *this) {
	...
	pthread_kill(this->thread_info[this->cur_th_num-1].thread_id, SIGKILL);
...
static void *_tp_work_thread(void *pthread) {
	...
	curid = pthread_self();                            //get current thread id
	
	nseq = this->get_thread_by_id(this, curid);        //get current thread's seq in the thread info struct array.
	if(nseq < 0)
		return;
	printf("entering working thread %u, thread id is %u\n", nseq, curid);
